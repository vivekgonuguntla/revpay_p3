...
public class MoneyRequestController {
    ...
    public ResponseEntity<MoneyRequestResponse> createRequest(
        ...
        return ResponseEntity.ok(moneyRequestService.createRequest(userId, dto, token));
    }

    @GetMapping
    public ResponseEntity<List<MoneyRequestResponse>> getRequests(
            @RequestHeader("X-User-Id") Long userId,
            @RequestHeader("Authorization") String token) {
        return ResponseEntity.ok(moneyRequestService.getRequests(userId, token));
    }
