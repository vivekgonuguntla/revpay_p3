...
public class CardServiceImpl implements CardService {
    ...
    public CardResponse addCard(Long userId, AddCardRequest request) {
        ...
        // Send notification
        try {
            sendAlertNotification(userId, "CARD_ADDED", "Card Added",
                "Card ending in " + lastFourDigits + " was added successfully.",
                "ACTIVE", savedCard.getId(), lastFourDigits);
        } catch (Exception e) {
    ...
    public void deleteCard(Long userId, Long cardId) {
        ...
        // Send notification
        try {
            sendAlertNotification(userId, "CARD_REMOVED", "Card Removed",
                "Card ending in " + lastFourDigits + " was removed.",
                "ACTIVE", null, lastFourDigits);
        } catch (Exception e) {
    ...
    public CardResponse setDefaultCard(Long userId, Long cardId) {
        ...
        // Send notification
        try {
            sendAlertNotification(userId, "DEFAULT_CARD_UPDATED", "Default Card Updated",
                "Card ending in " + savedCard.getLastFourDigits() + " is now your default card.",
                "ACTIVE", savedCard.getId(), savedCard.getLastFourDigits());
        } catch (Exception e) {
    ...
    @Override
    public boolean validateCard(Long userId, Long cardId) {
    ...
    private void sendAlertNotification(Long userId, String type, String title, String message,
                                       String eventStatus, Long cardId, String lastFourDigits) {
        NotificationServiceClient.NotificationRequest request = new NotificationServiceClient.NotificationRequest();
        request.userId = userId;
        request.category = "ALERTS";
        request.type = type;
        request.title = title;
        request.message = message;
        request.counterparty = lastFourDigits == null ? null : "Card •••• " + lastFourDigits;
        request.eventStatus = eventStatus;
        request.navigationTarget = "/cards";
        request.eventTime = java.time.LocalDateTime.now();
        Map<String, Object> metadata = new HashMap<>();
        if (cardId != null) {
            metadata.put("cardId", cardId);
        }
